-- Generate Narrative Routes from articles with same geopolitical topic
-- Run: psql -d veritas_app_development -f generate_routes_sql_v2.sql
...
-- Update total_hops and other derived fields for all routes
UPDATE narrative_routes 
SET total_hops = jsonb_array_length(hops),
    total_duration_seconds = COALESCE(
      EXTRACT(EPOCH FROM (
        (hops#>'{-1,published_at}')::timestamp - (hops#>'{0,published_at}')::timestamp
      ))::integer,
      0
    ),
